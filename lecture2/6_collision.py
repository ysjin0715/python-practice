...
# 적 캐릭터
enemy = pygame.image.load("C:/python/python-practice/lecture2/enemy.png")
enemy_size = enemy.get_rect().size  # 이미지의 크기를 본파일에서 가져와 리스트화한다.
enemy_width = enemy_size[0]  # 리스트화된 캐릭터의 크기정보 중 0번째를 가로크기로 정한다.
enemy_height = enemy_size[1]  # 리스트화된 캐릭터의 크기정보 중 1번째를 세로크기로 정한다.
enemy_x_pos = screen_width / 2 - enemy_width / 2  # 화면 가로의 절반 크기에 해당하는 곳에 위치한다. (x좌표)
enemy_y_pos = screen_height / 2 - enemy_height / 2 # 화면 세로크기 가장 아래에 해당하는 곳에 위치한다. (y좌표)
...
# 20fps : 1초 동안 20번 동작 -> 5 *20 = 100
   ...
   # 충돌 체크
   if character_rect.colliderect(enemy_rect):      # enemy와 충돌했는가
      print("충돌했어요")
      running(False)
